...
local function assert(condition, success, failure)
    return condition and WrapTextInColorCode(success, addon.SM.COLOR.GREEN) or WrapTextInColorCode(failure, addon.SM.COLOR.RED);
end

local SPEC = {};

local printed = List:new();
hooksecurefunc(addon.SM, "print", function(...)
    printed:push(List:new{...}:join(""));
end)

local CHAT_MSG_WHISPER_STUB = function(...) end
hooksecurefunc(addon.SM, "CHAT_MSG_WHISPER", function(...)
    CHAT_MSG_WHISPER_STUB(...);
end)

SPEC["SecureMessaging.should warn user"] = function(next)
    CHAT_MSG_WHISPER_STUB = function(...)
        next(assert(printed:includes(addon.SM.WARNING_MESSAGE), "Called print with warning message.", "Expected warning message."));
    end
...
SPEC["SecureMessaging.should allow verified whisper"] = function(next)
    CHAT_MSG_WHISPER_STUB = function(event, message)
        next(assert(message == "{rt3} groupie : passing test", "Received uninterrupted whisper.", "Expected uninterrupted whisper."));
    end
...
function SPEC.run()
    ...
    for name, test in pairs(SPEC) do
        if name ~= "run" then
            allTests:push({name = name, test = test});
        end
    end
...
function SlashCmdList.GROUPIE(args)
    ...
    if module:lower() == "sm" and command:lower() == "test" then
        SPEC.run();
    end
end
